...
@endif
    ...
    <h3 class="px-4 pb-4 fw-bold text-center">Halaman List Profil Lulusan</h3>
    
    <div class="container-fluid">
        <div class="card">
            <div class="card-body">
                @if(auth()->user()->otoritas === 'Penjamin Mutu')
                    <button type="submit" class="btn btn-success" onclick="create()">Add Graduate Profile</button>
                @endif
...
<script>
    ...
    // READ
    function read(){
        $.get("{{url('readListProfil')}}",{},function(data,status){
                // console.log(data);
              $('#read').html(data);
        });
    }
    ...
    function create(){
            $.get("{{url('createListProfil')}}",{},function(data,status){
                $("#exampleModalLabel").html("Add Graduate Profile").css("font-weight", "bold");
                $('#page').html(data);
    ...
    // PROSES SIMPAN
    function store() {
        var namaProfil  = $("#namaProfil").val();
        var deskripsi = $("#deskripsi").val();
        $.ajax({
            type: "GET",
            url: "{{url('storeListProfil')}}",
            data : {'namaProfil': namaProfil,'deskripsi': deskripsi},
    ...
    function showProfil(id) {
        $.get("{{ url('showProfil') }}/" + id, {}, function(data, status) {
            $("#exampleModalLabel").html("Edit Graduate Profile").css("font-weight", "bold");
            $('#page').html(data);
            $('#tambahModal').modal('show');
        });
    }
